...
//	trace()<<"CollisionModel "<<(int)par("collisionModel")<<self;
...
void Radio::readIniFileParameters(void)
{
	...
	if(self == 0){
		trace()<<"CollisionModel"<<self<<0;
		trace()<<"ModelCollision "<<(int)par("collisionModel");
	}
