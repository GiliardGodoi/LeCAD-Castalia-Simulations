...
Define_Module(Semaforo);

void Semaforo::startup()
{
...
void Semaforo::fromNetworkLayer(ApplicationPacket * rcvPacket, const char *source, double rssi, double lqi)
{
...
void Semaforo::timerFiredCallback(int index)
{
	switch (index) {
		case SEND_PACKET:{
...
void Semaforo::handleRadioControlMessage(RadioControlMessage *radioMsg)
{
...
void Semaforo::finishSpecific() {
	...
	for (int i = 0; i < numNodes; i++) {
		Semaforo *appModule = dynamic_cast<Semaforo*>(topo->getNode(i)->getModule()->getSubmodule("Application"));
		if (appModule) {
