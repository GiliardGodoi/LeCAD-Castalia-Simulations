...
#include "Basic8021542.h"

// This module is virtual and can not be used directly
Define_Module(Basic8021542); 

void Basic8021542::startup()
{
...
void Basic8021542::timerFiredCallback(int index)
{
	switch (index) {
		...
		case FRAME_START: {
			if (isPANCoordinator) {	// as a PAN coordinator, create and broadcast beacon packet
				beaconPacket = new Basic8021542Packet("PAN beacon packet", MAC_LAYER_PACKET);
 ...
 */
void Basic8021542::fromNetworkLayer(cPacket * pkt, int dstMacAddress)
{
	Basic8021542Packet *macPacket = new Basic8021542Packet("802.15.4 MAC data packet", MAC_LAYER_PACKET);
...
void Basic8021542::finishSpecific()
{
	if (currentPacket)
 ...
 */
void Basic8021542::setMacState(int newState)
{
...
Basic8021542Packet *Basic8021542::newConnectionRequest(int PANid) {
	Basic8021542Packet *result = new Basic8021542Packet("PAN associate request", MAC_LAYER_PACKET);
...
Basic8021542Packet *Basic8021542::newGtsRequest(int PANid, int slots) {
	Basic8021542Packet *result = new Basic8021542Packet("GTS request", MAC_LAYER_PACKET);
	result->setPANid(PANid);
	result->setDstID(PANid);
 ...
 */
void Basic8021542::fromRadioLayer(cPacket * pkt, double rssi, double lqi)
{
	Basic8021542Packet *rcvPacket = dynamic_cast<Basic8021542Packet*>(pkt);
	if (!rcvPacket) {
		return;
	}
	...
	switch (rcvPacket->getMac802154PacketType()) {
		...
		case MAC_802154_ASSOCIATE_PACKET:{
			...
			if (associationRequest_hub(rcvPacket)) {
				...
				Basic8021542Packet *ackPacket = new Basic8021542Packet("PAN associate response", MAC_LAYER_PACKET);
		...
		case MAC_802154_GTS_REQUEST_PACKET:{
			...
			// reply with an ACK
			Basic8021542Packet *ackPacket = new Basic8021542Packet("PAN GTS response", MAC_LAYER_PACKET);
			ackPacket->setPANid(SELF_MAC_ADDRESS);
			ackPacket->setMac802154PacketType(MAC_802154_ACK_PACKET);
			ackPacket->setDstID(rcvPacket->getSrcID());
			ackPacket->setByteLength(ACK_PKT_SIZE);
		...
		case MAC_802154_DATA_PACKET:{
			...
			Basic8021542Packet *ackPacket = new Basic8021542Packet("Ack packet", MAC_LAYER_PACKET);
			ackPacket->setPANid(SELF_MAC_ADDRESS);
			ackPacket->setMac802154PacketType(MAC_802154_ACK_PACKET);
			ackPacket->setDstID(rcvPacket->getSrcID());
...
void Basic8021542::handleAckPacket(Basic8021542Packet * rcvPacket)
{
 ...
 */
void Basic8021542::clearCurrentPacket(const char * s, bool success) {
	...
	Basic8021542Packet *tmpPacket = currentPacket;
	currentPacket = NULL;
...
void Basic8021542::transmitPacket(Basic8021542Packet *pkt, int retries, bool state, double limit) {
	clearCurrentPacket();
...
void Basic8021542::attemptTransmission(const char * descr)
{
...
void Basic8021542::performCSMACA()
{
...
void Basic8021542::transmitCurrentPacket()
{
	if (currentPacket == NULL) {
...
void Basic8021542::collectPacketHistory(const char *s)
{
	if (!currentPacket) {
...
bool Basic8021542::acceptNewPacket(Basic8021542Packet *newPacket) 
{
...
void Basic8021542::receiveBeacon_node(Basic8021542Packet *beacon) 
{
...
void Basic8021542::transmissionOutcome(Basic8021542Packet *pkt, bool success, string history) 
{
	if (getAssociatedPAN() != -1 && TXBuffer.size()) {
		Basic8021542Packet *packet = check_and_cast<Basic8021542Packet*>(TXBuffer.front());
		TXBuffer.pop();
